...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / f'military_frontier_{get_beijing_time().strftime("%Y%m")}.log'),
...
class MilitaryFrontierAutomation:
    """军事前沿自动化处理类"""
...
## 第四步：并行启动 military_frontier_editor agent

为每个筛选出的主题启动一个 military_frontier_editor agent，传递主题描述。

每个 agent 需要：
1. 使用 WebSearch 深入搜索该事件详细信息
2. 查找技术规格、战略分析、专家评论
3. 用 curl 下载 5-8 张高质量图片到 `./images/`（武器照片、战术图、性能数据）
4. 写一篇 1500-2000 字的军事分析文章（含 frontmatter）
...
**文章要求**：
- 标题吸引眼球（使用"揭秘"、"对比"、"分析"等词汇）
- 包含技术细节、战略意义、未来影响
- 武器装备类：技术原理、性能参数、作战效能
- 地缘态势类：力量对比、战略意图、影响分析
- 实战案例类：战术演变、经验教训、技术验证
- 1500-2000字，5-8张配图
- 结尾引导互动（开放性问题 + 点赞理由 + 分享动机）

**文章主题示例**：
- 武器装备: "高超音速导弹为何难防：3个技术细节揭秘"
- 地缘态势: "南海局势突变：这次演习传递了什么信号"
- 实战案例: "俄乌战争2年：改变现代战争的5个教训"
- 军事演习: "史上最大规模：北约这次演习暴露3个战略意图"
...
**重要**：不要询问确认，直接执行所有步骤。并行运行所有 agent 提高效率。"""
    ...
    def run_claude_code(self):
        ...
        try:
            logger.info("="*60)
            logger.info(f"开始执行 军事前沿 自动化任务 - {get_beijing_time()}")
...
def run_now(article_count: int, verbose: bool = False):
    """立即执行任务"""
    logger.info("🚀 立即执行模式")
    automation = MilitaryFrontierAutomation(article_count=article_count, verbose=verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='军事前沿自动化生成与发布系统',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 立即生成 1 篇文章
  python daily_military_frontier.py --now --count 1

  # 立即生成 1 篇文章（实时显示执行过程）
  python daily_military_frontier.py --now --count 1 --verbose

  # 立即生成 3 篇文章
  python daily_military_frontier.py --now --count 3

  # 定时每天 08:00 生成 5 篇文章
  python daily_military_frontier.py --count 5

  # 定时每天 14:00 生成 2 篇文章（实时输出）
  python daily_military_frontier.py --time 14:00 --count 2 -v

  # 定时每天早上8点和晚上8点各生成 10 篇文章
  python daily_military_frontier.py --time 08:00 20:00 --count 10

  # 定时多个时间点生成文章（早中晚）
  python daily_military_frontier.py --time 08:00 12:00 20:00 --count 5
        """
    )
