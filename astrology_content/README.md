...
本目录用于生产“安夏式星座短文号”本地草稿：每日 2 篇单星座短文，加 1 篇十二星座日运；标题有钩子，正文稳健原创。
...
生成安夏短文号 7 天单星座排期，默认每日 2 篇：

```bash
python3 anxia_calendar.py --days 7 --daily 2 --profile anxia_short
```

排期会读取 `reviews/performance.jsonl`。某个“星座 × 主题”至少有 3 篇有效表现记录后，才会在不打破星座均衡的前提下获得优先级；可用 `--performance-min-samples` 调整门槛。
单日运行也会按日期继续十二星座和三类主题轮播，不会每天重新从同一组星座开始，也不会因默认每日 2 篇而长期漏掉“财运/贵人”。单星座文章以安夏星座为主对标；“十二星座每日好运”保持独立生成规则。

一步生成 3 天文章量（默认每天 2 篇单星座稿 + 1 篇十二星座日运，共 9 篇），并自动质检：

```bash
python3 anxia_generate.py --days 3
```

生成器会同时写入 `reviews/editorial/` 中与文章同名的编辑记录。每篇记录保留 4 个与当前正文变体一致的标题版本、2 个开头版本和当前默认选项；标题另行标注内容公式与数字清单/单点悬念/趋势预告/风险提醒/场景判断等点击表达。发布时可在表现录入命令中标记实际采用的版本。记录绑定当前文章摘要，文章修改后需要重新生成或更新记录，避免选题卡、标题备选和实际成稿脱节。
...
默认知识库路径为 `D:\自媒体\知识库\01-公众号文章\星座公众号文章\安夏星座`，默认模式为 `viral-safe`。`--daily 2` 表示每天 2 篇单星座短文，日运默认额外增加 1 篇；总计每天 3 篇。若当天文件已存在，会自动跳过；需要重写已存在草稿时添加 `--overwrite`。
...
`report` 中的标题公式、标题表达和标题版本按加权阅读率排序；其他维度优先看互动率。任一分组少于 3 篇时会显示“样本不足”，只用于继续积累，不直接调整排期权重。
