...
主题：AI加持超级个体、Prompt/Context Engineering、AI工具教程、独立创业者案例、数字游民签证政策
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / f'nomad_news_{datetime.now().strftime("%Y%m")}.log'),
...
class DigitalNomadNewsAutomation:
    """数字游民资讯自动化处理类"""
...
## 第一步：并行搜索 Google News（过去24小时）

使用 WebFetch 并行搜索以下关键词的 Google News（过去24小时），提取新闻标题、来源和链接：

**搜索 URL 格式**：`https://www.google.com/search?q=[关键词]&tbm=nws&tbs=qdr:d`
...
4. "AI agent workflow"
5. "Claude AI productivity"
6. "Cursor AI coding"
7. "indie hacker AI"
8. "one person business AI"
9. "MCP servers"
10. "digital nomad visa 2024"
...
**筛选标准**：
- 必须是过去24小时内的新闻
- 必须与已有文章主题不重复（不只是标题不同，主题要真正不同）
- 优先级：Prompt/Context Engineering > AI工具教程 > 超级个体案例 > 数字游民签证

**如果去重后不足 {self.article_count} 个主题，就写实际找到的数量，宁缺毋滥**

## 第四步：并行启动 digital-nomad-editor agent

为每个筛选出的主题启动一个 digital-nomad-editor agent，传递主题描述。

每个 agent 需要：
1. 使用 WebSearch 深入搜索该主题详细信息
2. 用 curl 下载 5-8 张高质量图片到 `./images/`
3. 写一篇 1500-2500 字的公众号文章（含 frontmatter）
4. 保存为 `./articles/YYYYMMDD_HHMM_topic.md`
5. **立即**使用 `mcp__wenyan-mcp__publish_article_from_file` 发布到草稿箱（选择 agentera 系列主题）
6. **发布成功后**，提取文章中的所有图片路径，用 `mcp__wenyan-mcp__publish_image_message` 发布图片消息（简短评论1-2句话 + 3-5个话题标签）

**文章主题示例**：
- Prompt Engineering: "Claude 3.5 Sonnet 的 Context Engineering 最佳实践"
- AI 工具: "Cursor AI 编程完整教程：从入门到精通"
- 超级个体: "独立开发者如何用 AI 实现月入 $10K"
- 数字游民: "2024 日本数字游民签证申请全攻略"
...
**重要**：不要询问确认，直接执行所有步骤。并行运行所有 agent 提高效率。"""
    ...
    def run_claude_code(self):
        ...
        try:
            logger.info("="*60)
            logger.info(f"开始执行 AI 新闻自动化任务 - {datetime.now()}")
...
def run_now(article_count: int, verbose: bool = False):
    """立即执行任务"""
    logger.info("🚀 立即执行模式")
    automation = DigitalNomadNewsAutomation(article_count=article_count, verbose=verbose)
...
def run_scheduled(article_count: int, schedule_times: list = None, verbose: bool = False):
    ...
    logger.info(f"文章数量: {article_count} 篇（每次执行）")
    ...
    def job():
        logger.info(f"🚀 定时任务触发 - 北京时间: {datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')}")
        automation = DigitalNomadNewsAutomation(article_count=article_count, verbose=verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='数字游民资讯自动化生成与发布系统',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 立即生成 1 篇文章
  python daily_nomad_news.py --now --count 1

  # 立即生成 1 篇文章（实时显示执行过程）
  python daily_nomad_news.py --now --count 1 --verbose

  # 立即生成 3 篇文章
  python daily_nomad_news.py --now --count 3

  # 定时每天 08:00 生成 5 篇文章
  python daily_nomad_news.py --count 5

  # 定时每天 14:00 生成 2 篇文章（实时输出）
  python daily_nomad_news.py --time 14:00 --count 2 -v

  # 定时每天 09:00 和 19:00 各生成 10 篇文章
  python daily_nomad_news.py --time 09:00,19:00 --count 10
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='08:00',
        help='定时执行时间，格式 HH:MM，支持多个时间用逗号分隔（例如: 09:00,19:00；默认: 08:00）'
    )
    ...
    # 显示配置
    logger.info("="*60)
    logger.info("数字游民资讯自动化系统")
    logger.info("="*60)
    logger.info(f"文章数量: {args.count} 篇")
    logger.info(f"执行模式: {'立即执行' if args.now else f'定时执行 ({args.time})'}")
    ...
    if args.now:
        ...
        # 解析多个时间点
        schedule_times = [t.strip() for t in args.time.split(',')]
        return run_scheduled(args.count, schedule_times, args.verbose)
