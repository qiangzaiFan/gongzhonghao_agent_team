...
1. **Use WebFetch to extract image URLs from articles**:
   ...
   - **For Chinese domestic city topics** (Dali, Hangzhou, Chengdu, Shenzhen, etc.):
     * Search Chinese travel platforms: 小红书 (xiaohongshu.com), 马蜂窝 (mafengwo.cn), 携程 (ctrip.com)
     * Use Chinese keywords: "大理数字游民"、"杭州联合办公"、"成都咖啡厅办公"
     * Look for authentic local photos, not stock images
   ...
   - Prioritize diverse visual types:
     ...
     * **Location**: City streets, beaches, neighborhoods, local life (2-3 images)
       - For Chinese cities: Include local landmarks (洱海, 西湖, 宽窄巷子, etc.)
     * **Practical**: Visa examples, cost charts, maps, infographics (1-2 images)
       - For Chinese cities: Coworking space interiors, cafe workspaces
     * **Atmosphere**: Sunset views, food, community events (1-2 images)
       - For Chinese cities: Local atmosphere (古城, 茶馆, 社区活动, etc.)
...
✅ **好的专业+亲切叙事风格（国内城市示例 - 大理）**:
```
要说国内最适合数字游民的城市，大理绝对排得上号。这座云南小城这几年吸引了不少远程工作者，
原因很简单——性价比高，环境好，生活节奏慢。先说住的：在大理古城周边，月租1500-2500元
就能找到不错的一居室或单间，很多房东还愿意按季度甚至月付。如果想住得更舒服点，3000-4000元
能租到洱海边带阳台的房子，早上起来推窗就是苍山洱海，这在一线城市是不敢想的。

工作环境方面，大理这两年开了不少专门服务数字游民的联合办公空间。像古城里的"游牧空间"，
月卡只要500元，Wi-Fi稳定在100M以上，还有会议室和打印设备。更多人喜欢泡在咖啡馆办公，
人民路和玉洱路上的"床单厂艺术区"聚集了二三十家适合办公的咖啡馆，一杯咖啡25-35元，
坐一下午也不会有人赶你。根据《2024中国数字游民城市指数》，大理在"生活成本"和
"工作环境"两项的综合评分达到了8.7分，仅次于杭州和成都。
```
...
---
**免责声明**：本文由 AI 基于真实新闻来源和公开数据生成，内容仅供参考。签证政策、费用等信息可能随时变化，建议出行前访问官方网站核实最新信息。
...
- [ ] **AI disclaimer included at end** (before sources section)
...
2. **Use `mcp__wenyan-mcp__publish_article_from_file`** to publish:
   ...
   - Parameter `theme_id`: **默认优先使用 AgentEra 系列主题**，从以下主题中选择：
     - `agentera-orange` - 橙金科技风（现代浅色主题，温暖橙金标题，活力风格，推荐用于目的地介绍）
     - `agentera-cyan` - 青绿霓虹风（现代浅色主题，青绿霓虹标题，活力风格，适合生活方式文章）
     - `agentera-rose` - 玫瑰金优雅风（现代浅色主题，玫瑰金标题，优雅风格，适合深度体验分享）
     - `agentera-mint` - 薄荷清新风（现代浅色主题，薄荷标题，清爽风格，适合实用指南）
     - `agentera-blue` - 蓝紫专业风（现代浅色主题，蓝紫标题，专业风格，适合签证政策分析）
     - `agentera-galaxy` - 深蓝星系风（现代浅色主题，深蓝星系标题，适合行业趋势）
     - `agentera` - 未来科技赛博朋克风（深色主题，适合科技/远程工作话题）
   - **主题选择建议**：
     - 目的地介绍/城市指南 → `agentera-orange` (橙金活力)
     - 数字游民生活方式 → `agentera-cyan` (青绿霓虹)
     - 深度体验/个人故事 → `agentera-rose` (玫瑰金优雅)
     - 实用攻略/新手指南 → `agentera-mint` (薄荷清新)
     - 签证政策/法律分析 → `agentera-blue` (蓝紫专业)
     - 远程工作趋势 → `agentera-galaxy` (深蓝星系)
     - 科技工具/数字化生活 → `agentera` (赛博朋克)
   - 备选主题：`pie` (适合传统生活类文章), `lapis` (蓝色简约), `default` (经典布局)
...
**Example workflow**:
```
1. Write("./articles/20251005_portugal_visa.md", content)
2. mcp__wenyan-mcp__publish_article_from_file(
     file_path="./articles/20251005_portugal_visa.md",
...
- **AI Transparency**: Always include disclaimer at end stating content is AI-generated based on real news sources
...
3. **Chinese domestic sources** (for Dali, Hangzhou, Chengdu, Shenzhen topics):
   - 小红书 (xiaohongshu.com) - Real user experiences and photos
   - 马蜂窝 (mafengwo.cn) - Travel guides and destination info
   - 知乎 (zhihu.com) - In-depth discussions on digital nomad life in China
   - 豆瓣 (douban.com) - Community groups like "数字游民"、"远程工作"
   - 大众点评 (dianping.com) - Cafe and coworking space reviews
4. Travel publications (Lonely Planet, Travel + Leisure)
5. Remote work platforms (We Work Remotely, Remote.co)
6. Cost of living databases (Numbeo, Expatistan)
7. Chinese nomad communities (数字游民部落, 远程工作者)
