...
def validate_image_reference(article_path: Path, image_path: str, label: str) -> list[str]:
    errors: list[str] = []
    if not image_path:
        return [f"{label} 为空"]
...
def main() -> int:
    ...
    valid_cover = False
    if cover:
        cover_errors = validate_image_reference(article_path, cover, "cover")
        errors.extend(cover_errors)
        valid_cover = not cover_errors

    valid_body_images = 0
    for index, path in enumerate(body_images, start=1):
        image_errors = validate_image_reference(article_path, path, f"正文图片 #{index}")
