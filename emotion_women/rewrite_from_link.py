...
## 工作要求
1. 先分析来源文章为什么容易被读完：具体矛盾、读者处境、信息推进和有效细节。
2. 再进行原创转化：
   - 保留的只能是“大众痛点/情绪矛盾/选题方向”；
   - 必须更换标题、开头、论证顺序、小标题和比喻；来源没有可复用事实时，不另编人物和故事；
...
## 文章硬性要求
- 正文目标 800 个中文字符，必须在 720-900 个中文字符之间。
- 标题 20 字以内，聚焦一个爆点。
- 标题从新文章里最具体的冲突或动作提炼，不能像来源标题，不套固定标题公式。
- frontmatter 只写 title。
- 固定 2 个二级小标题，标题对应故事里正在发生的具体事件。
- 只使用来源支持的事实和场景，不新增“朋友/读者/咨询对象”冒充真人素材。
- 固定加粗 1 处，必须来自人物现场对话或具体物件，不用普适道理充当金句。
- 故事占正文至少 75%，作者分析最多两小段；结尾停在动作、对话或物件上，不解释故事意义。
- 保持模板数量，但开头、加粗位置、图片落点、冲突结果和结尾不得复用近期文章骨架。
- 唯一加粗句禁止写成“有些女人/真正的关系/好的婚姻”等万能判断；人物改变后保留一个实际后果。
- 结尾自然停住，最多留 1 个具体问题，不同时索要评论、点赞和转发。
- 文末不要列参考资料/来源链接。
- 不要出现这些模型化表达：每个人都值得被爱、时间会治愈一切、我们应该认识到、综上所述、由此可见、女性要学会、正确的做法是、这告诉我们、在这个快节奏的时代、你有没有发现、请相信。
...
2. 保留来源能够支持的具体动作、生活物件和对话，减少抽象判断；不要为了生动新增事实。
3. 删除或改写模板化表达、正确废话、口号式鸡汤。
4. 调整段落节奏：长短随内容变化，避免每段都像“观点-解释-总结”，也不要连续一句一段。
5. 保持标题 20 字以内；从正文最具体的冲突或动作提炼，不套固定标题公式，也不要像来源标题。
6. 不要新增无法确认的真实人物、真实数据、真实新闻事实。
7. 不要把来源文章逐段换词；如果发现初稿和来源结构太像，要重排段落和论述重心。
8. 删除三连建议、排比升华、连续反问、“不是……而是……”串联和互动三件套。
...
- 故事占正文至少 75%，作者分析最多两小段；结尾停在动作、对话或物件上，不解释故事意义。
- 模板数量不变，但重新安排加粗、图片、转折和结尾位置，避免与近期文章使用同一段落功能顺序。
- 删除万能加粗句和过分顺滑的觉醒；人物改变后至少保留一个实际后果。
- 结尾最多留 1 个具体问题，不同时索要评论、点赞和转发。
- 文末不列来源、参考资料或链接。

## 避免这些 AI 感表达
每个人都值得被爱、时间会治愈一切、我们应该认识到、综上所述、由此可见、女性要学会、正确的做法是、这告诉我们、在这个快节奏时代、你有没有发现、请相信、愿每一个女孩。
