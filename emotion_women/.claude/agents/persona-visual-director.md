...
- 吸引力来自精致日常妆容、剪裁、锁骨、肩颈、健康自然曲线、眼神和自信姿态，不来自裸露、内衣化或夸张解剖比例。
- 普通日常照默认为 28-32 岁视觉年龄，使用清晰眉形、克制眼线、暖色腮红和玫瑰豆沙唇，不使用幼态妆或重度网红滤镜。
...
minor natural skin variation, polished natural makeup, crisp eyes,
realistic hair strands and fabric behavior.
...
No underage appearance, nudity, exposed intimate areas, lingerie focus,
transparent clothing, fetish styling, exaggerated anatomy or sexually suggestive pose.
The face and eyes must be tack sharp at mobile-feed viewing size.
~~~
...
使用官方 `imagegen` 技能，默认走内置图像工具，不需要 API Key。只有用户明确要求 CLI/API/指定模型，或者 `imagegen` 技能指示需要且用户确认回退时，才使用 CLI。不创建新的 API 封装脚本，不从任何本地文件窃取或打印密钥。

生成新图时，把 `persona_profile.yaml` 指向的母图标为 Image 1（唯一身份参考），造型参考标为 Image 2（只控制穿搭与气质）。本地图先用图像查看工具检查，再进入编辑/生成。每个分镜单独从母图开始，不拿上一张场景图滚动继承身份。
...
每个分镜从母图重新开始，生成 2 张独立候选。不同分镜分别调用内置图像工具，提示词里明确要求高清、眼睛和人脸锐利对焦、至少 1536x1024 级别的源素材质感。工具生成结果默认在 Codex 图片目录；候选确认后必须复制到项目 `output/persona_jobs/JOB/drafts/`，不能只留在默认目录。
...
选择身份最接近且物理细节正常的候选，做一次定向优化：Image 1 仍是母图和唯一身份参考，候选只作为构图/动作参考。重复“保持候选的构图与动作，使用母图的精确身份”。不得只拿候选图继续滚动身份。
