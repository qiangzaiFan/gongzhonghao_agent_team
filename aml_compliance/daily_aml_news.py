...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / f'aml_news_{get_beijing_time().strftime("%Y%m")}.log'),
...
4. "cryptocurrency AML"
5. "anti-money laundering"
6. "sanctions compliance"
7. "correspondent banking"
8. "cross-border e-commerce sanctions"
9. "PBOC anti-money laundering"
10. "crypto sanctions"
...
**筛选标准**：
- 必须是过去24-48小时内的新闻（AML合规新闻周期较长）
- 必须与已有文章主题不重复
- 优先级：监管政策 > 重大罚款 > AI技术应用 > 加密货币监管

**重点关注**：
- OFAC/FATF/FinCEN官方公告
- 重大AML罚款案例
- 朝鲜制裁更新
- 加密货币监管动态
- 代理行合规要求
...
## 第四步：并行启动 aml-compliance-editor agent

为每个筛选出的主题启动一个 aml-compliance-editor agent，传递主题描述。

每个 agent 需要：
1. 使用 WebSearch 深入搜索该事件详细信息
2. 查找官方文件、监管公告、专业分析
3. 用 curl 下载 3-5 张高质量图片到 `./images/`（官方logo、监管文件、数据图表）
4. 写一篇 1500-2500 字的AML合规分析文章（含 frontmatter）
5. 保存为 `./articles/YYYYMMDD_HHMM_topic.md`
6. **立即**使用 `mcp__wenyan-mcp__publish_article_from_file` 发布到草稿箱（选择 agentera-blue 主题）
7. **发布成功后**，提取文章中的所有图片路径，用 `mcp__wenyan-mcp__publish_image_message` 发布图片消息（简短评论1-2句话 + 3-5个话题标签）

**文章要求**：
- 标题专业准确（使用"OFAC"、"制裁"、"反洗钱"等关键词）
- 监管政策类：包含政策原文、影响分析、合规建议
- 罚款案例类：包含案件背景、违规细节、合规启示
- 加密货币类：包含监管要求、技术方案、行业影响
- 1500-2500字，3-5张配图
- 结尾引导互动（开放性问题 + 点赞理由 + 分享动机）

**文章主题示例**：
- 监管动态: "OFAC新增15家俄罗斯实体至SDN清单：金融机构筛查要点"
- 罚款案例: "币安因KYC缺陷被罚43亿美元：加密交易所合规启示"
- 朝鲜制裁: "联合国更新朝鲜制裁清单：代理行如何应对"
- 技术应用: "NLP技术提升制裁筛查准确率：实施指南"
...
**重要**：不要询问确认，直接执行所有步骤。并行运行所有 agent 提高效率。"""
    ...
    def run_claude_code(self):
        ...
        try:
            logger.info("="*60)
            logger.info(f"开始执行 AML合规资讯自动化任务 - {get_beijing_time()}")
...
def run_now(article_count: int, verbose: bool = False):
    """立即执行任务"""
    logger.info("🚀 立即执行模式")
    automation = AMLNewsAutomation(article_count=article_count, verbose=verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='AML合规资讯自动化生成与发布系统',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 立即生成 1 篇文章
  python daily_aml_news.py --now --count 1

  # 立即生成 1 篇文章（实时显示执行过程）
  python daily_aml_news.py --now --count 1 --verbose

  # 立即生成 3 篇文章
  python daily_aml_news.py --now --count 3

  # 定时每天 08:00 生成 5 篇文章
  python daily_aml_news.py --count 5

  # 定时每天 14:00 生成 2 篇文章（实时输出）
  python daily_aml_news.py --time 14:00 --count 2 -v

  # 定时每天早上8点和晚上8点各生成 10 篇文章
  python daily_aml_news.py --time 08:00 20:00 --count 10

  # 定时多个时间点生成文章（早中晚）
  python daily_aml_news.py --time 08:00 12:00 20:00 --count 5
        """
    )
